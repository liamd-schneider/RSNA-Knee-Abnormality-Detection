"""RSNA Knee Abnormality Detection -- baseline training kernel (v10).

v5's unweighted, 1,500-study, 6-epoch recipe remains this project's best
result (val macro AUC 0.700) after v6-v9 all tried variations on data
volume, epoch count, and loss weighting and landed at or below it. v10
leaves training exactly alone and tests a different kind of lever
entirely: test-time augmentation (TTA) at evaluation, not training.

Every prediction so far has come from a single forward pass over each
series' slices exactly as decoded. TTA instead runs each series through
the model TTA_VIEWS times -- once on the original slices, the rest through
a small random rigid jitter (a few degrees of rotation, +-5% scale, a
small translation, the same augmentation family the reference notebooks
used for their own training-time jitter) -- and averages the sigmoid
probabilities across views. The idea: a single fixed crop/framing is one
sample of "what the model thinks," and averaging several nearby samples
should be a less noisy estimate, the same reasoning behind averaging
several models in an ensemble, just applied to input views of one model
instead of several trained models.

Purely an inference-time change -- training is identical to v5 (loss
weights both left at 1.0, i.e. no weighting, matching v5/v6/v7's
unweighted setup; MAX_EXTRA/EPOCHS at v5's 1,500/6) so any score
difference is attributable to TTA specifically, not a silent training
change. Applied to both validation (to measure whether it helps) and the
final test-set submission (since if it helps, the actual deliverable
should use it too).

Carries forward v7's best-checkpoint saving and v6's per-epoch history
checkpointing + time budget safety net. Same model/data pipeline
otherwise:
...
LR_HEAD = 1e-3
RUN_NAME = "tta"
EPOCHS = int(os.environ.get("KNEE_EPOCHS", "6"))       # back to v5's settings -- this run tests TTA, not training
VAL_FRAC = 0.2
SEED = 42
...
# v8/v9 tested loss weighting and it was a net negative -- both left at 1.0
# here (no weighting) to reproduce v5's training exactly.
...
# Test-time augmentation: TTA_VIEWS-1 randomly jittered views averaged in
# with the original, unaugmented prediction (view 0). Same rigid-jitter
# family as the reference notebooks' training-time augmentation, applied
# here at evaluation instead.
...
CACHE_DIR = os.environ.get("KNEE_CACHE_DIR", os.path.join(tempfile.gettempdir(), f"knee_tensor_cache_v10_{RUN_NAME}"))
...
def tta_jitter(x):
    ...
    at evaluation instead. x: (S, 1, H, W)."""
...
def predict_study(model, series_list, tta_views=1):
    ...
    unaugmented original). Arithmetic mean of probabilities, not logits --
    an arithmetic mean of risk, matching the reference notebooks' reasoning
    for TTA-window pooling."""
    probs = []
...
def main():
    ...
    ckpt_path = os.path.join(OUT_DIR, f"model_v10_{RUN_NAME}.pt")
    history_path = os.path.join(OUT_DIR, f"history_v10_{RUN_NAME}.json")
    ...
    # Same model, evaluated two ways: tta_views=1 is the plain single-pass
    # prediction (directly comparable to v5-v9's numbers, no new variable
    # introduced there), tta_views=TTA_VIEWS is the actual thing v10 is
    # testing. Isolates the TTA effect since both numbers come from the
    # identical trained weights -- only the inference procedure differs.
    no_tta_auc, no_tta_n, no_tta_per_label = evaluate(model, val_ds, device, tta_views=1)
    final_val_auc, final_n, final_per_label = evaluate(model, val_ds, device, tta_views=TTA_VIEWS)
    print(f"[done] no-TTA val macro AUC: {no_tta_auc}", flush=True)
    print(f"[done] TTA (views={TTA_VIEWS}) val macro AUC: {final_val_auc}", flush=True)
    save_history(history, best_auc, best_epoch, final_per_label, stopped_early, no_tta_auc=no_tta_auc)
    print(f"[done] final per-label val AUC (TTA): {final_per_label}", flush=True)
    ...
    if os.path.exists(test_csv):
        ...
        with torch.no_grad():
            for i in range(len(test_ds)):
                ...
                if series_list:
                    series_list = [s.to(device) for s in series_list]
                    probs = predict_study(model, series_list, tta_views=TTA_VIEWS).cpu().numpy()[0]
                else:
