"""RSNA Knee Abnormality Detection -- baseline training kernel (v11).

v5's unweighted, 1,500-study, 6-epoch recipe (0.700) is still this
project's best training result after v6-v9's data/epoch/weighting
experiments; v10 separately showed test-time augmentation adds a real,
isolated +0.011 on top of it. v11 tests a different, purely
preprocessing-level idea, isolated from both of those on purpose (v5's
plain training, no TTA) so its own effect is clean to read.

Physical-scale cropping. Every study so far has been resized to 224x224
regardless of the scanner's pixel spacing (mm/pixel varies across this
corpus's mix of scanners/protocols) -- so a knee filling most of the
frame on one scanner and a third of the frame on another both get treated
identically by the resize, with the model never told the difference.
CROP_MM (130mm, the reference notebooks' finding for this corpus: below
the field of view of ~99.6% of series while still containing the joint)
is now cropped from the image centre, using each series' PixelSpacing
DICOM tag to convert millimetres to pixels, *before* the resize -- so the
knee occupies a comparable fraction of the frame across scanners instead
of whatever the acquisition's raw pixel grid happened to produce.

Carries forward v7's best-checkpoint saving and v6's per-epoch history
checkpointing + time budget safety net.

Same model/data pipeline otherwise as v5, so any score difference is
attributable to physical-scale cropping specifically, not a silent
architecture or training change:
...
LR_HEAD = 1e-3
RUN_NAME = "mmcrop"
EPOCHS = int(os.environ.get("KNEE_EPOCHS", "6"))       # v5's settings -- this run tests physical cropping, not training
VAL_FRAC = 0.2
SEED = 42
...
# weighting) here to reproduce v5's training exactly.
...
# Physical-scale crop: a fixed millimetre field of view around the image
# centre, converted to pixels via each series' own PixelSpacing, applied
# before the resize to IMG_SIZE -- see the reference notebooks' finding
# (module docstring) for why 130mm.
CROP_MM = 130.0

OUT_DIR = os.environ.get("KNEE_OUT_DIR", "/kaggle/working")
CACHE_DIR = os.environ.get("KNEE_CACHE_DIR", os.path.join(tempfile.gettempdir(), f"knee_tensor_cache_v11_{RUN_NAME}"))
...
def main():
    ...
    ckpt_path = os.path.join(OUT_DIR, f"model_v11_{RUN_NAME}.pt")
    history_path = os.path.join(OUT_DIR, f"history_v11_{RUN_NAME}.json")
