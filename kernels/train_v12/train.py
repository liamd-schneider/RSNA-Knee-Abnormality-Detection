"""RSNA Knee Abnormality Detection -- baseline training kernel (v12, ensemble).

v10 measured something important almost by accident: two runs with v5's
*exact* training configuration -- same data, same epochs, same everything
-- landed 0.033 AUC apart (0.700 vs 0.667) purely from unseeded random
weight initialization. v11's mm-crop result (0.719) was a promising number
that couldn't be trusted against that noise band with only one run each.

v12 addresses both problems the same way, on purpose: train N_MODELS
independently-seeded models (torch.manual_seed per member, actually fixed
this time -- model init was never seeded before this run) on the same
data, decoded once and shared across all members via the disk cache, each
using the current best-guess recipe (v5's training settings + v11's
physical-scale crop + v10's TTA at evaluation). Two things fall out of
this for free:

1. **An honest measurement of the noise band**, at last -- N independent
   val AUCs from the identical recipe directly show how much run-to-run
   spread is chance vs. signal, instead of inferring it indirectly like
   v10/v11 had to.
2. **An actual ensemble**: predictions from all N members are averaged
   (percentile rank, not raw probability -- the metric reads only order,
   and rank-averaging doesn't let one miscalibrated member's absolute
   probabilities dominate the vote) for both the validation comparison and
   the final test-set submission. Averaging independent models' errors out
   is the standard reason ensembling helps, and it's the same reasoning
   TTA already validated (v10) at the input-view level instead of the
   model level.

A time budget check now gates starting each *model*, not just each epoch
-- N_MODELS models is N_MODELS times v5's training cost, and this is the
first run in the project actually at risk of not finishing everything it
set out to do inside one Kaggle session.
...
LR_HEAD = 1e-3
RUN_NAME = "ensemble"
EPOCHS = int(os.environ.get("KNEE_EPOCHS", "6"))       # v5's settings, per ensemble member
VAL_FRAC = 0.2
...
# Ensemble members. Each gets its own torch.manual_seed -- model init was
# never seeded before this run (v10 found the gap; this is the fix).
...
CACHE_DIR = os.environ.get("KNEE_CACHE_DIR", os.path.join(tempfile.gettempdir(), f"knee_tensor_cache_v12_{RUN_NAME}"))
...
def main():
    ...
    def train_one_member(member_idx, model_seed):
        ...
        ckpt_path = os.path.join(OUT_DIR, f"model_v12_member{member_idx}.pt")
        history_path = os.path.join(OUT_DIR, f"history_v12_member{member_idx}.json")
    ...
    with open(os.path.join(OUT_DIR, "history_v12_ensemble_summary.json"), "w") as f:
        json.dump({
