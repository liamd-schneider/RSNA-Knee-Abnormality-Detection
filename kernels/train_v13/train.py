"""RSNA Knee Abnormality Detection -- baseline training kernel (v13).

v12's 3-member ensemble (v5's recipe + v11's crop + v10's TTA, DINOv2-small)
gave this project its first trustworthy number: 0.711, directly accounting
for the ~0.027 AUC spread that turned out to come from nothing but random
weight initialization. That's the best-supported estimate so far, but not
the ceiling -- v13 pushes the two levers most likely to move it further,
having already ruled out (v6-v9) that more data, more epochs, or loss
weighting do it on their own:

1. **A bigger backbone.** DINOV2_MODEL switched from dinov2-small (~22M
   params) to dinov2-base (~86M) -- more representational capacity for the
   same fine-tuning recipe (still only the last UNFREEZE_LAST transformer
   blocks trainable). This is the single lever this project's analysis of
   the ~0.9-scoring reference notebooks flagged as most likely to matter
   and hasn't been tried yet.
2. **A slightly longer per-member epoch budget** (6 -> 10, still well
   under v7-moreepochs' measured ~11-epoch overfitting point for this data
   size) -- safe to raise because every member already keeps its
   best-validation-AUC checkpoint, not whatever epoch training ends on, so
   a few extra epochs are pure upside (more chances to find a better peak)
   with no new overfitting risk from the checkpoint mechanism itself.

Both are real compute-cost increases (a ~4x larger backbone, per member,
for more epochs) stacked on top of N_MODELS members -- the time budget
check (already gating each model's start since v12) matters more here
than in any run before it.

Carries forward v7's best-checkpoint saving and v6's per-epoch history
checkpointing. Same model/data pipeline per member otherwise:

1. A pretrained backbone (DINOv2, via Hugging Face transformers) instead
   of v1-v4's from-scratch CNN. Only the last UNFREEZE_LAST transformer
   blocks + the final layernorm are trainable, at a much lower
...
DINOV2_MODEL = os.environ.get("KNEE_DINOV2_MODEL", "facebook/dinov2-base")  # up from -small
...
LR_BACKBONE = 8e-6
LR_HEAD = 1e-3
RUN_NAME = "ensemble_base"
EPOCHS = int(os.environ.get("KNEE_EPOCHS", "10"))      # up from v5's 6 -- see module docstring
VAL_FRAC = 0.2
SEED = 42          # for the data split / extracted-study sample -- NOT the model init seeds below
MAX_EXTRA = int(os.environ.get("KNEE_MAX_EXTRA", "1500"))  # v5's settings
...
# v8/v9 showed loss weighting is a net negative -- both left at 1.0 (no
# weighting) to reproduce v5's training exactly.
GOLD_LOSS_WEIGHT = float(os.environ.get("KNEE_GOLD_WEIGHT", "1.0"))
EXTRACTED_LOSS_WEIGHT = float(os.environ.get("KNEE_EXTRACTED_WEIGHT", "1.0"))

# Physical-scale crop (v11): a fixed millimetre field of view around the
# image centre, converted to pixels via each series' own PixelSpacing,
# applied before the resize to IMG_SIZE.
CROP_MM = 130.0

# Test-time augmentation (v10), applied per ensemble member's prediction.
TTA_VIEWS = int(os.environ.get("KNEE_TTA_VIEWS", "5"))
TTA_ROT_DEG = 5.0
TTA_SCALE = 0.05
TTA_SHIFT = 0.03

# Ensemble members. Each gets its own torch.manual_seed (v12's fix, kept).
# Same count as v12 -- a bigger backbone already raises per-member cost,
# so member count stays fixed while the backbone lever gets tested cleanly.
N_MODELS = int(os.environ.get("KNEE_N_MODELS", "3"))
MODEL_SEEDS = [1000 + i for i in range(N_MODELS)]

OUT_DIR = os.environ.get("KNEE_OUT_DIR", "/kaggle/working")
CACHE_DIR = os.environ.get("KNEE_CACHE_DIR", os.path.join(tempfile.gettempdir(), f"knee_tensor_cache_v13_{RUN_NAME}"))
...
def read_series(series_dir, plane, laterality, img_size=IMG_SIZE, max_slices=MAX_SLICES):
    ...
    # Physical-scale crop before anything resolution-dependent: a fixed mm
    # field of view converted to this series' own pixel grid, so 130mm of
    # tissue occupies the same fraction of the frame regardless of scanner.
    # Skipped (not forced) when the crop would be degenerate -- larger than
    # the image (nothing to crop) or implausibly small (a bad PixelSpacing
    # value), same sanity check the reference notebooks used.
...
# ---------------------------------------------------------------- eval loop
def tta_jitter(x):
    """A small random rigid warp (rotation/scale/shift) -- same augmentation
    family as the reference notebooks' training-time jitter, applied here
    at evaluation instead (v10). x: (S, 1, H, W)."""
    n, dev = x.shape[0], x.device
    rot = (torch.rand(n, device=dev) - 0.5) * 2 * (TTA_ROT_DEG * np.pi / 180)
    sc = 1.0 + torch.rand(n, device=dev) * TTA_SCALE
    tx = (torch.rand(n, device=dev) - 0.5) * 2 * TTA_SHIFT
    ty = (torch.rand(n, device=dev) - 0.5) * 2 * TTA_SHIFT
...
def predict_study(model, series_list, tta_views=1):
    """Average sigmoid probabilities over tta_views (view 0 is always the
    unaugmented original)."""
    probs = []
    for v in range(tta_views):
        view = series_list if v == 0 else [tta_jitter(s) for s in series_list]
        logits = model.forward_batch([view])
        probs.append(torch.sigmoid(logits))
    return torch.stack(probs).mean(dim=0)


def predict_all(model, ds, device, tta_views=1):
    """One member's predictions over every item in ds. Returns
    {StudyInstanceUID: probs (NUM_LABELS,) numpy array}, plus labels for
    the studies that have them (empty dict for the unlabeled test set)."""
    model.eval()
    probs_by_uid, labels_by_uid = {}, {}
    with torch.no_grad():
        for i in range(len(ds)):
            ...
            probs = predict_study(model, series_list, tta_views=tta_views)
            probs_by_uid[uid] = probs.cpu().numpy()[0]
            labels_by_uid[uid] = label.numpy()
    return probs_by_uid, labels_by_uid


def score_probs(probs_by_uid, labels_by_uid):
    """macro AUC / per-label AUC from a {uid: probs} / {uid: label} pair."""
    uids = sorted(set(probs_by_uid) & set(labels_by_uid))
    if not uids:
        return None, 0, {}
    probs = np.stack([probs_by_uid[u] for u in uids])
    labels = np.stack([labels_by_uid[u] for u in uids])
    per_label = {}
...
def evaluate(model, ds, device, tta_views=1):
    """Thin wrapper over predict_all+score_probs for the per-epoch
    monitoring loop (always tta_views=1 there -- cheap, comparable to
    v5-v11's per-epoch numbers)."""
    probs_by_uid, labels_by_uid = predict_all(model, ds, device, tta_views=tta_views)
    return score_probs(probs_by_uid, labels_by_uid)


def rank_average(prob_dicts):
    """Percentile-rank mean across members, per the reference notebooks'
    reasoning: the metric reads only order, so ranking each member's
    predictions before averaging keeps one miscalibrated member's raw
    probability scale from dominating the vote."""
    uids = sorted(set.union(*(set(d) for d in prob_dicts)))
    acc = np.zeros((len(uids), NUM_LABELS), dtype=np.float64)
    n = 0
    for d in prob_dicts:
        have = [u for u in uids if u in d]
        if not have:
            continue
        mat = np.stack([d[u] for u in have])
        ranks = pd.DataFrame(mat).rank(pct=True).to_numpy()
        idx = [uids.index(u) for u in have]
        acc[idx] += ranks
        n += 1
    return {u: acc[i] / max(n, 1) for i, u in enumerate(uids)}
...
def main():
    ...
    print(f"[setup] torch={torch.__version__} device={device} epochs={EPOCHS} "
          ...
          f"n_models={N_MODELS} tta_views={TTA_VIEWS} crop_mm={CROP_MM}", flush=True)
    ...
    # One dataset/cache shared across every ensemble member -- decoded once,
    # reused N_MODELS times, same reasoning as v3's original disk cache but
    # now amortized across whole models instead of just epochs.
    ...
    def train_one_member(member_idx, model_seed):
        torch.manual_seed(model_seed)
        model = KneeMILModel().to(device)
        if member_idx == 0:
            n_trainable = sum(p.numel() for p in model.parameters() if p.requires_grad)
            n_total = sum(p.numel() for p in model.parameters())
            print(f"[setup] model: {n_trainable/1e6:.1f}M / {n_total/1e6:.1f}M params trainable",
                  flush=True)
        ...
        ckpt_path = os.path.join(OUT_DIR, f"model_v13_member{member_idx}.pt")
        history_path = os.path.join(OUT_DIR, f"history_v13_member{member_idx}.json")
        ...
        def save_history(history, best_auc, best_epoch, final_per_label, stopped_early,
            ...
            with open(history_path, "w") as f:
                json.dump({"history": history, "best_val_macro_auc": best_auc,
                            ...
                            "member_idx": member_idx, "model_seed": model_seed,
                            "no_tta_final_val_macro_auc": no_tta_auc,
        ...
        def save_checkpoint(state_dict):
            torch.save({"model": state_dict, "img_size": IMG_SIZE,
                        ...
                        "dinov2_model": DINOV2_MODEL, "model_seed": model_seed}, ckpt_path)

        history, best_auc, best_epoch, best_state, stopped_early = [], -1.0, -1, None, False
        for epoch in range(EPOCHS):
            if time.time() - t0 > TIME_BUDGET_S:
                print(f"[time-budget] {TIME_BUDGET_S:.0f}s reached before member "
                      f"{member_idx} epoch {epoch+1} -- stopping this member's training "
            ...
            for i in range(len(train_ds)):
                try:
                    ...
                    print(f"  [warn] member {member_idx} study {i} failed "
                          f"({type(e).__name__}: {e}), skipping", flush=True)
                    continue

                if (i + 1) % 200 == 0:
                    print(f"  [member {member_idx} epoch {epoch+1}] {i+1}/{len(train_ds)} "
                          f"studies, elapsed={time.time()-t0:.0f}s", flush=True)
            ...
            print(f"[member {member_idx} epoch {epoch+1}/{EPOCHS}] train_loss={train_loss:.4f} "
                  f"val_macro_auc={auc_str} elapsed={time.time()-t0:.0f}s", flush=True)
        ...
        no_tta_auc, _, _ = evaluate(model, val_ds, device, tta_views=1)
        val_probs, val_labels = predict_all(model, val_ds, device, tta_views=TTA_VIEWS)
        tta_auc, _, tta_per_label = score_probs(val_probs, val_labels)
        save_history(history, best_auc, best_epoch, tta_per_label, stopped_early,
                      no_tta_auc=no_tta_auc)
        print(f"[member {member_idx}] done: best_epoch_auc={best_auc:.4f} "
              f"no_tta_final={no_tta_auc} tta_final={tta_auc}", flush=True)

        test_probs = {}
        if test_ds is not None:
            test_probs, _ = predict_all(model, test_ds, device, tta_views=TTA_VIEWS)

        return {"member_idx": member_idx, "model_seed": model_seed, "best_epoch_auc": best_auc,
                "no_tta_final_auc": no_tta_auc, "tta_final_auc": tta_auc,
                "val_probs": val_probs, "val_labels": val_labels, "test_probs": test_probs}

    members = []
    for idx, seed in enumerate(MODEL_SEEDS):
        if time.time() - t0 > TIME_BUDGET_S:
            print(f"[time-budget] stopping before member {idx} -- ensembling the "
                  f"{len(members)} member(s) already trained", flush=True)
            break
        print(f"[ensemble] training member {idx+1}/{N_MODELS} (seed={seed})...", flush=True)
        members.append(train_one_member(idx, seed))

    # Ensemble: rank-average every trained member's validation predictions,
    # score against the (shared, identical across members) val labels.
    ensemble_val_probs = rank_average([m["val_probs"] for m in members])
    ensemble_auc, ensemble_n, ensemble_per_label = score_probs(ensemble_val_probs, members[0]["val_labels"])

    individual_aucs = [m["tta_final_auc"] for m in members]
    print(f"[ensemble] individual member TTA AUCs: {individual_aucs}", flush=True)
    print(f"[ensemble] mean individual AUC: {np.mean(individual_aucs):.4f}, "
          f"spread: {np.max(individual_aucs)-np.min(individual_aucs):.4f}", flush=True)
    print(f"[ensemble] ensemble AUC ({len(members)} members): {ensemble_auc}", flush=True)

    with open(os.path.join(OUT_DIR, "history_v13_ensemble_summary.json"), "w") as f:
        json.dump({
            "n_models_configured": N_MODELS, "n_models_trained": len(members),
            "model_seeds": [m["model_seed"] for m in members],
            "individual_best_epoch_auc": [m["best_epoch_auc"] for m in members],
            "individual_no_tta_final_auc": [m["no_tta_final_auc"] for m in members],
            "individual_tta_final_auc": individual_aucs,
            "mean_individual_auc": float(np.mean(individual_aucs)) if individual_aucs else None,
            "individual_auc_spread": float(np.max(individual_aucs) - np.min(individual_aucs))
                                      if len(individual_aucs) > 1 else None,
            "ensemble_val_macro_auc": ensemble_auc, "ensemble_n_labels": ensemble_n,
            "ensemble_per_label_auc": ensemble_per_label,
            "n_train": len(train_df), "n_val": len(val_df),
            "n_extracted_used": len(extracted), "max_extra": MAX_EXTRA,
            "tta_views": TTA_VIEWS, "crop_mm": CROP_MM,
            "torch_version": torch.__version__, "device": device,
        }, f, indent=2)
    print(f"[done] ensemble summary saved", flush=True)

    if test_ds is not None and members:
        ensemble_test_probs = rank_average([m["test_probs"] for m in members])
        rows = []
        for uid, probs in ensemble_test_probs.items():
        ...
        print(f"[done] wrote {sub_path} ({len(rows)} studies, "
              f"{len(members)}-member ensemble)", flush=True)
