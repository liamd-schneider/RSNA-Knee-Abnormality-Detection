"""RSNA Knee Abnormality Detection -- baseline training kernel (v14).

v12 (DINOv2-small, 3 members): ensemble 0.711. v13 (DINOv2-base, 3
members, more epochs): ensemble 0.752. Both times, ensembling itself
delivered a real gain over the mean individual member (v12: 0.709 mean ->
0.711 ensemble; v13: 0.728 mean -> 0.752 ensemble) -- and neither run has
tested whether *more* members compounds that further, since both used
the same N_MODELS=3. v14 is that direct test: v13's exact recipe
(DINOv2-base, 10 epochs, crop, TTA), N_MODELS raised from 3 to 5.

The two new members (seeds 1003, 1004) are additional draws from the same
noise distribution v12/v13 already measured (spread ~0.03-0.04 AUC from
weight init alone) -- more independent draws to rank-average should
further reduce the ensemble's exposure to any single unlucky member,
which is the entire mechanism ensembling relies on to help at all.

Real compute-cost increase again: 5 members instead of 3 at DINOv2-base's
per-member cost is the most expensive run this project has queued yet.
The time-budget check (gating each model's start since v12) is a hard
requirement here, not just a safety margin.
...
# Raised from v12/v13's 3 -- this run's whole point is testing whether more
# independent draws compounds the ensembling gain further.
N_MODELS = int(os.environ.get("KNEE_N_MODELS", "5"))
...
def main():
    ...
    def train_one_member(member_idx, model_seed):
        ...
        ckpt_path = os.path.join(OUT_DIR, f"model_v14_member{member_idx}.pt")
        history_path = os.path.join(OUT_DIR, f"history_v14_member{member_idx}.json")
    ...
    with open(os.path.join(OUT_DIR, "history_v14_ensemble_summary.json"), "w") as f:
        json.dump({
