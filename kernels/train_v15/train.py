"""RSNA Knee Abnormality Detection -- baseline training kernel (v15).

v14 tried to test whether 5 ensemble members beat 3 (v12/v13 both used
N_MODELS=3), at v13's 10-epochs-per-member budget. It never got to
answer that: the 7h time budget triggered after 3 of 5 members, stopping
cleanly as designed but leaving the actual question open. That 3-member
partial run did land a second data point close to v13's (0.758 vs.
0.752) -- reassuring that ~0.75 is a stable estimate for this recipe, but
not what v14 set out to measure.

v15 tries again with the arithmetic fixed instead of the time budget
raised: v14's 3 members finished 3x10=30 "member-epochs" inside the
budget, so 5 members at 6 epochs each (5x6=30 member-epochs, v12's
original epoch count) targets the same total compute that already fit,
instead of hoping a bigger time allowance also fits. If DINOv2-base
still benefits meaningfully from 10 epochs over 6, this trades some of
v13's per-member quality for actually getting all 5 independent draws
into one ensemble -- an open question the run itself will answer.
...
LR_BACKBONE = 8e-6
LR_HEAD = 1e-3
RUN_NAME = "ensemble_base5_6ep"
EPOCHS = int(os.environ.get("KNEE_EPOCHS", "6"))       # back to v5/v12's count -- see module docstring
VAL_FRAC = 0.2
...
def main():
    ...
    def train_one_member(member_idx, model_seed):
        ...
        ckpt_path = os.path.join(OUT_DIR, f"model_v15_member{member_idx}.pt")
        history_path = os.path.join(OUT_DIR, f"history_v15_member{member_idx}.json")
    ...
    with open(os.path.join(OUT_DIR, "history_v15_ensemble_summary.json"), "w") as f:
        json.dump({
