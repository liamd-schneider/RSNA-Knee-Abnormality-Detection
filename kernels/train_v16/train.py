"""RSNA Knee Abnormality Detection -- baseline training kernel (v16).

v15 settled a question v13/v14 raised: DINOv2-base's gain over -small
comes from epoch count, not ensemble member count -- 3 members at 10
epochs (v13/v14: 0.752/0.758) clearly beat 5 members at 6 epochs (v15:
0.711, flat vs. v12's 0.711 despite 67% more members). v16 acts on that
directly: v13's exact recipe, N_MODELS back down to 3 (matches the proven
time-budget shape), EPOCHS pushed further, 10 -> 14, to see whether the
epoch lever keeps paying off or has started to plateau/overfit.

This is genuinely unmeasured territory: v7-moreepochs' ~11-epoch
overfitting point was measured on the *from-scratch CNN* recipe (v1-v4),
not DINOv2-base with only the last 6 transformer blocks trainable --
those two setups have no reason to share an overfitting epoch count, and
this project doesn't have a base-model epoch curve yet to check against.
Safe to try regardless: every member keeps its best-val-AUC checkpoint,
so extra epochs are pure upside even if the true peak comes earlier.
...
LR_BACKBONE = 8e-6
LR_HEAD = 1e-3
RUN_NAME = "ensemble_base_14ep"
EPOCHS = int(os.environ.get("KNEE_EPOCHS", "14"))      # up from v13's 10 -- see module docstring
VAL_FRAC = 0.2
...
def main():
    ...
    def train_one_member(member_idx, model_seed):
        ...
        ckpt_path = os.path.join(OUT_DIR, f"model_v16_member{member_idx}.pt")
        history_path = os.path.join(OUT_DIR, f"history_v16_member{member_idx}.json")
    ...
    with open(os.path.join(OUT_DIR, "history_v16_ensemble_summary.json"), "w") as f:
        json.dump({
