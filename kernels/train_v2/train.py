"""RSNA Knee Abnormality Detection -- baseline training kernel (v2).

Same model/data as v1 (kernels/train_v1), plus the piece v1 was missing: a
held-out validation split so we get an honest per-label ROC AUC instead of
only a training loss. 58 labeled studies is tiny, so this split is a plain
80/20 random split (not stratified -- with 12 labels at once there isn't a
clean way to stratify all of them from ~58 rows), and several labels will
likely have too few positives in the ~12-study validation set for AUC to even
be defined (needs both classes present) -- that's reported explicitly rather
than silently averaged away.

Local dry run:
    KNEE_INPUT_DIR=data/dryrun KNEE_OUT_DIR=/tmp/out KNEE_EPOCHS=1 python train.py
"""
import glob
import json
import os
...
def main():
    ...
    ckpt_path = os.path.join(OUT_DIR, "model_v2.pt")
    ...
    with open(os.path.join(OUT_DIR, "history_v2.json"), "w") as f:
