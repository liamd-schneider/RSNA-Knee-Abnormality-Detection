"""RSNA Knee Abnormality Detection -- baseline training kernel (v5).

Same data as v4 (46 gold + up to 1,500 report-extracted studies, validated
only on the 12 held-out gold studies) -- deliberately unchanged, so any
score difference from v4 can be attributed to what actually changed here:
...
LR_BACKBONE = 8e-6
LR_HEAD = 1e-3
EPOCHS = int(os.environ.get("KNEE_EPOCHS", "6"))
VAL_FRAC = 0.2
SEED = 42
MAX_EXTRA = int(os.environ.get("KNEE_MAX_EXTRA", "1500"))  # unchanged from v4 on purpose
LAT_DEAD_ZONE_MM = 20.0

OUT_DIR = os.environ.get("KNEE_OUT_DIR", "/kaggle/working")
CACHE_DIR = os.environ.get("KNEE_CACHE_DIR", os.path.join(tempfile.gettempdir(), "knee_tensor_cache_v5"))
...
def main():
    ...
    ckpt_path = os.path.join(OUT_DIR, "model_v5.pt")
    ...
    with open(os.path.join(OUT_DIR, "history_v5.json"), "w") as f:
