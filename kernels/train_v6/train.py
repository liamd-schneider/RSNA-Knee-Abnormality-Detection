"""RSNA Knee Abnormality Detection -- baseline training kernel (v6).

v5 (DINOv2 + geometric slice order + laterality normalization) took val
macro AUC from v4's 0.541 to 0.700, and was still climbing at its last
epoch (6) -- confirmation the recipe works, not yet the ceiling of it.
v6 scales the same recipe up along the two cheapest, lowest-risk levers:

1. More data: MAX_EXTRA raised from v5's 1,500 to all ~3,980 usable
   extracted-label studies (parse failures excluded, same as before).
2. More epochs: default raised from 6 to 15, since v5 hadn't plateaued.

Scaling both at once is a real risk after v4's two crashes, so this run
also adds the safety net v4/v5 didn't have:

- A wall-clock time budget (TIME_BUDGET_S, default 7h): if training is
  still running past it, the epoch loop stops cleanly at the *next*
  epoch boundary and everything gets saved anyway, rather than the
  session limit killing the process mid-epoch with nothing recoverable.
- history_v6.json is now written after *every* epoch, not only once at
  the very end -- if this run does die unexpectedly, whatever epochs
  completed are still on disk as kernel output, not lost entirely (the
  exact gap flagged in the extract_labels_v1_full writeup).

Same model/data otherwise as v5, so a score change is still attributable
to "more data + more epochs" and not a silent architecture change:
...
LR_BACKBONE = 8e-6
LR_HEAD = 1e-3
EPOCHS = int(os.environ.get("KNEE_EPOCHS", "15"))  # v5 hadn't plateaued at 6
VAL_FRAC = 0.2
SEED = 42
MAX_EXTRA = int(os.environ.get("KNEE_MAX_EXTRA", "4000"))  # ~3,980 usable -> effectively "use all"
...
CACHE_DIR = os.environ.get("KNEE_CACHE_DIR", os.path.join(tempfile.gettempdir(), "knee_tensor_cache_v6"))
...
def main():
    ...
    ckpt_path = os.path.join(OUT_DIR, "model_v6.pt")
    history_path = os.path.join(OUT_DIR, "history_v6.json")

    def save_checkpoint(history, best_auc, best_epoch, final_per_label, stopped_early):
    ...
    for epoch in range(EPOCHS):
        ...
        # Checkpoint after every epoch, not just at the end -- if this run
        ...
        save_checkpoint(history, best_auc, best_epoch, per_label, stopped_early)
    ...
    save_checkpoint(history, best_auc, best_epoch, final_per_label, stopped_early)
