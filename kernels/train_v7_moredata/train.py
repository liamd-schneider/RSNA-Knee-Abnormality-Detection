"""RSNA Knee Abnormality Detection -- baseline training kernel (v7, "moredata" variant).

v5 (DINOv2 + geometric slice order + laterality normalization, 1,500
extracted studies, 6 epochs) hit val macro AUC 0.700, still climbing at
its last epoch. v6 scaled BOTH data (1,500 -> ~3,980 extracted studies)
and epochs (6 -> 15) at once, and came back *worse* (0.660) -- train loss
fell to 0.069 while held-out AUC just oscillated 0.62-0.66, the
overfitting pattern back in a new form. Changing two variables at once
meant v6 couldn't say which one caused the regression, or whether it
needed both together.

v7 is two separate, one-variable-at-a-time reruns to actually answer that:

- **train_v7_moredata** (this file): v5's epoch count (6), v6's data
  volume (~3,980 extracted studies). Isolates "more data alone."
- **train_v7_moreepochs** (kernels/train_v7_moreepochs/train.py, same
  code, MAX_EXTRA/EPOCHS defaults swapped): v5's data volume (1,500),
  v6's epoch count (15). Isolates "more epochs alone."

Also fixes a real gap v6 exposed: the saved model checkpoint used to be
whichever epoch the run happened to *end* on, not the epoch with the best
validation AUC -- harmless in v5/v6 by coincidence, not something worth
relying on going forward. Now the best-scoring epoch's weights are kept in
memory and only that state is written to the final checkpoint;
history_v7.json still records every epoch for the full curve.

Carries forward v6's per-epoch history checkpointing and time budget
(TIME_BUDGET_S, default 7h -- stop cleanly at an epoch boundary rather
than risk a mid-epoch kill) since both variants hit the same real corpus.

Same model/data pipeline otherwise as v5/v6, so any score difference is
attributable to the one deliberately-changed variable, not a silent
...
LR_BACKBONE = 8e-6
LR_HEAD = 1e-3
RUN_NAME = "moredata"  # isolates "more data alone" -- v5's epoch count, v6's data volume
EPOCHS = int(os.environ.get("KNEE_EPOCHS", "6"))       # unchanged from v5 on purpose
VAL_FRAC = 0.2
SEED = 42
MAX_EXTRA = int(os.environ.get("KNEE_MAX_EXTRA", "4000"))  # ~3,980 usable, same as v6
...
CACHE_DIR = os.environ.get("KNEE_CACHE_DIR", os.path.join(tempfile.gettempdir(), f"knee_tensor_cache_v7_{RUN_NAME}"))
...
def main():
    ...
    print(f"[setup] torch={torch.__version__} device={device} epochs={EPOCHS} "
          f"max_extra={MAX_EXTRA} model={DINOV2_MODEL} cache_dir={CACHE_DIR}", flush=True)
    ...
    ckpt_path = os.path.join(OUT_DIR, f"model_v7_{RUN_NAME}.pt")
    history_path = os.path.join(OUT_DIR, f"history_v7_{RUN_NAME}.json")
