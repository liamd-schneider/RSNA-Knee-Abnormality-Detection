"""RSNA Knee Abnormality Detection -- baseline training kernel (v8).

v7 isolated v6's regression: more extracted-label data alone measurably
hurt (0.700 -> 0.683), more epochs alone barely moved the needle (0.700 ->
0.696), and combining both compounded the damage (0.660). Read together:
the ~3,980 extracted studies are real signal (validated at 75.9% label
accuracy / 0.64 F1 against the 58 gold studies) but noisy enough that
training pours proportionally more gradient signal into fitting their
~24% wrong labels the more of them are in the mix and the longer training
runs -- every study's loss counts equally right now, gold or extracted.

v8 tests the direct fix: weight each study's loss by its label source
(GOLD_LOSS_WEIGHT vs EXTRACTED_LOSS_WEIGHT, both configurable) instead of
treating a gold-labeled study and an extracted-label study as equally
trustworthy. Back on v5/v7-moredata's settings otherwise (6 epochs; MAX_EXTRA
still configurable, default keeps v5's 1,500 since that's the best recipe
found so far and this run is testing one specific new idea against it, not
re-opening the data-volume question already answered by v7).
...
Same model/data pipeline otherwise as v5/v6/v7, so any score difference is
attributable to loss weighting specifically, not a silent architecture or
...
LR_BACKBONE = 8e-6
LR_HEAD = 1e-3
RUN_NAME = "lossweighted"
EPOCHS = int(os.environ.get("KNEE_EPOCHS", "6"))       # v5's settings -- this run tests loss weighting, not data volume
VAL_FRAC = 0.2
SEED = 42
...
CACHE_DIR = os.environ.get("KNEE_CACHE_DIR", os.path.join(tempfile.gettempdir(), f"knee_tensor_cache_v8_{RUN_NAME}"))
...
def main():
    ...
    ckpt_path = os.path.join(OUT_DIR, f"model_v8_{RUN_NAME}.pt")
    history_path = os.path.join(OUT_DIR, f"history_v8_{RUN_NAME}.json")
